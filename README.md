...
`renovate.json` configura il regex manager nativo con datasource `git-refs`: estrae lo SHA completo (`currentDigest`) senza impostare `currentValue`. In questa modalità il datasource risolve il `HEAD` remoto, cioè il branch predefinito upstream. Non impostare `currentValueTemplate: "HEAD"`: il datasource lo cercherebbe come nome di branch o tag. Il riferimento osservato non è un riferimento mobile usato dall'installer; anche un cambio del branch predefinito produce soltanto una proposta da revisionare.
